...
class OrderAdmin(admin.ModelAdmin):
    ...
    def export_selected_ojects(self, request, queryset):
        ...
        for row in queryset.all():
            ...
            writer.writerow([row.order_txt, products_txt[:-2].encode('utf8'), row.realname.encode('utf8') if row.realname else '', row.phone, row.address.encode('utf8') if row.address else '', row.message.encode('utf8') if row.message else '', row.create_at, row.status])
